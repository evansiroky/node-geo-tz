...
var intersection = function(a, b) {
  ...
  result = geoJsonParser.write(result);
...
module.exports = function(tzGeojson, callback) {
  ...
  while(curPctIndexed < 99) {
    ...
    for(i = 0; i < curZones.length; i++) {
      ...
      if(curZones.tzs) {
        ...
        for (var j = 0; j < tzGeojson.features.length; j++) {
  ...
  // write data to file
  var writeStream = fs.createWriteStream(DATA_DIR + 'index.json')

  writeStream.end(JSON.stringify(data), function(err) {
    if(err) {
      return callback(err)
    }
    callback()
